(() => {
    ...
    function isVerticalText(el) {
        ...
        return wm === "vertical-rl" || wm === "vertical-lr" || el.dataset.autofitVertical === "true";
    }

    function fits(el) {
        // Bei writing-mode: vertical-* wächst der Text entlang der Höhe.
        // Der Browser tauscht scrollWidth/scrollHeight dabei NICHT —
        // Overflow erscheint auf der Block-Achse (scrollHeight).
        if (isVerticalText(el)) {
    ...
    function computeMaxPx(el) {
        ...
        // Bei vertikalem Text (writing-mode) sind Höhe und Breite vertauscht
        if (isVerticalText(el)) {
            return Math.max(12, Math.floor(Math.min(w * 0.95, h * 0.60)));
        }
